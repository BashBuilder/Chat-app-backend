...
import { HttpError, InternalServerError } from '@chatapp/common';
...
export const userProxyService = {
  ...
  async searchUsers(payload: SearchUsersPayload): Promise<UserSearchResponse> {
    try {
      const response = await client.get<UserSearchResponse>('/users/search', {
        headers: authHeader.headers,
        params: {
          query: payload.query,
          ...(payload.limit && { limit: payload.limit }),
          ...(payload.excludeIds && { excludeIds: payload.excludeIds }),
        },
      });
      return response.data;
    } catch (error) {
